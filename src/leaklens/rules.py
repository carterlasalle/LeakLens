...
def builtin_rules() -> tuple[Rule, ...]:
    ...
    return (
        _rule(
            ...
            critical,
            tags=("key", "asymmetric"),
            message="Private key material begins here",
            keywords=("private key",),
        ),
        _rule(
            "github-pat",
            "GitHub personal access token",
            r"(?<![A-Za-z0-9_])(?P<secret>github_pat_[A-Za-z0-9_]{20,255}|gh[pousr]_[A-Za-z0-9]{36,255})(?![A-Za-z0-9_])",
            critical,
            tags=("github", "token"),
            keywords=("github_pat_", "ghp_", "gho_", "ghu_", "ghs_", "ghr_"),
        ),
        _rule(
            "gitlab-pat",
            "GitLab personal access token",
            r"(?<![A-Za-z0-9_-])(?P<secret>glpat-[A-Za-z0-9_-]{20,255})(?![A-Za-z0-9_-])",
            critical,
            tags=("gitlab", "token"),
            keywords=("glpat-",),
        ),
        ...
        _rule(
            "google-api-key",
            "Google API key",
            r"(?<![A-Za-z0-9_-])(?P<secret>AIza[A-Za-z0-9_-]{35})(?![A-Za-z0-9_-])",
            high,
            tags=("google", "cloud", "token"),
            keywords=("aiza",),
        ),
        _rule(
            "slack-token",
            "Slack token",
            r"(?<![A-Za-z0-9-])(?P<secret>xox[baprs]-[A-Za-z0-9-]{10,250})(?![A-Za-z0-9-])",
            critical,
            tags=("slack", "token"),
            keywords=("xox",),
        ),
        _rule(
            ...
            r"(?<![A-Za-z0-9_])(?P<secret>(?:sk|rk)_live_[A-Za-z0-9]{20,255})(?![A-Za-z0-9_])",
            critical,
        ...
        _rule(
            "openai-api-key",
            "OpenAI API key",
            r"(?<![A-Za-z0-9_-])(?P<secret>sk-(?!ant-)(?:proj-|svcacct-)?[A-Za-z0-9_-]{20,255})(?![A-Za-z0-9_-])",
            critical,
            entropy=3.0,
            tags=("openai", "ai", "token"),
            keywords=("sk-",),
        ),
        _rule(
            "anthropic-api-key",
            "Anthropic API key",
            r"(?<![A-Za-z0-9_-])(?P<secret>sk-ant-[A-Za-z0-9_-]{20,255})(?![A-Za-z0-9_-])",
            critical,
            entropy=3.0,
            tags=("anthropic", "ai", "token"),
            keywords=("sk-ant-",),
        ),
        _rule(
            "sendgrid-api-key",
            "SendGrid API key",
            r"(?<![A-Za-z0-9_.-])(?P<secret>SG\.[A-Za-z0-9_-]{16,32}\.[A-Za-z0-9_-]{32,64})(?![A-Za-z0-9_.-])",
            critical,
            tags=("sendgrid", "email", "token"),
            keywords=("sg.",),
        ),
        _rule(
            "npm-access-token",
            ...
            critical,
            tags=("npm", "supply-chain", "token"),
            keywords=("npm_",),
        ),
        _rule(
            "pypi-upload-token",
            "PyPI upload token",
            r"(?<![A-Za-z0-9_-])(?P<secret>pypi-[A-Za-z0-9_-]{50,255})(?![A-Za-z0-9_-])",
            critical,
            tags=("pypi", "supply-chain", "token"),
            keywords=("pypi-",),
        ),
        _rule(
            "jwt",
            "JSON Web Token",
            r"(?<![A-Za-z0-9_-])(?P<secret>eyJ[A-Za-z0-9_-]{4,}\.[A-Za-z0-9_-]{4,}\.[A-Za-z0-9_-]{4,})(?![A-Za-z0-9_-])",
            high,
            entropy=3.0,
            tags=("jwt", "token"),
            message="JWT-like bearer credential",
            keywords=("eyj",),
        ),
        _rule(
            "database-url",
            "Database connection URL",
            r"(?P<secret>(?:postgres(?:ql)?|mysql|mariadb|mongodb(?:\+srv)?|redis)://[^\s:'\"]+:[^\s@'\"]+@[^\s'\"]+)",
            critical,
            tags=("database", "password"),
            message="Connection URL contains inline credentials",
            keywords=(
                "postgres://",
                "postgresql://",
                "mysql://",
        ...
        _rule(
            ...
            high,
            tags=("url", "password"),
            message="URL contains inline credentials",
            keywords=("http://", "https://"),
        ),
        _rule(
            "generic-secret",
            "Generic assigned secret",
            r"(?:api[_-]?key|client[_-]?secret|access[_-]?token|auth[_-]?token|password|passwd|secret)\s*[=:]\s*['\"](?P<secret>[^'\"\r\n]{8,512})['\"]",
            high,
            confidence="medium",
            entropy=3.0,
