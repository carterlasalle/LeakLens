...
from leaklens.engine import Scanner  # noqa: E402


def main() -> None:
    line = "const configuration = { enabled: true, retries: 3 };\n"
    payload = line * 100_000 + 'password = "Q9!wE8@rT7#y"\n'
    scanner = Scanner()
    started = time.perf_counter()
    result = scanner.scan_text(payload, path="benchmark.js")
    elapsed = time.perf_counter() - started
    mib = len(payload.encode()) / 1024 / 1024
    print(f"Scanned {mib:.2f} MiB in {elapsed:.3f}s ({mib / elapsed:.1f} MiB/s); findings={len(result.findings)}")
